        ...
        .on("close", function () {
          ...
          fs.createReadStream("./site/archive.zip")
            ...
            .on("close", function () {
              ...
              helpers
                .moveDir("./site/espocrm-" + branch.replace("/", "-"), "./site")
                .then(function () {
                  resolve();
                });
            })
...
function install() {
  return new Promise(function (resolve) {
    ...
    try {
      ...
      cp.execSync(
        'php install/cli.php -a step1 -d "user-lang=' +
      ...
      console.log(
        ex,
        `==========> ERR : exec php install/cli.php -a step1 -d "user-lang="`
      );
    }
    ...
    try {
      ...
      cp.execSync(
        'php install/cli.php -a setupConfirmation -d "host-name=' +
      ...
      console.log(ex, `==========> ERR : setupConfirmation`);
    }
    ...
    try {
      ...
      console.log(ex, `==========> ERR : checkPermission`);
    }
    ...
    try {
      ...
      cp.execSync(
        'php install/cli.php -a saveSettings -d "site-url=' +
          config.install.siteUrl +
          "&default-permissions-user=" +
          config.install.defaultOwner +
          "&default-permissions-group=" +
          config.install.defaultGroup +
          '"',
        { cwd: "./site" }
      );
    } catch (ex) {
      console.log(ex, `==========> ERR : saveSettings`);
    }
    ...
    try {
      ...
      console.log(ex, `==========> ERR : buildDatabase`);
    }
    ...
    try {
      ...
      cp.execSync(
        'php install/cli.php -a createUser -d "user-name=' +
          config.install.adminUsername +
          "&user-pass=" +
          config.install.adminPassword +
          '"',
        { cwd: "./site" }
      );
    } catch (ex) {
      console.log(ex, `==========> ERR : createUser`);
    }
    ...
    try {
      ...
      console.log(ex, `==========> ERR : Merge configs`);
    }
...
function copyExtension() {
  return new Promise(function (resolve, fail) {
    ...
    if (fs.existsSync("./site/application/Espo/Modules/" + moduleName)) {
      ...
      helpers.deleteDirRecursively(
        "./site/application/Espo/Modules/" + moduleName
      );
    }
    ...
    if (fs.existsSync("./site/tests/unit/Espo/Modules/" + moduleName)) {
      ...
      helpers.deleteDirRecursively(
        "./site/tests/unit/Espo/Modules/" + moduleName
      );
    }
    ...
    if (fs.existsSync("./site/tests/integration/Espo/Modules/" + moduleName)) {
      ...
      helpers.deleteDirRecursively(
        "./site/tests/integration/Espo/Modules/" + moduleName
      );
    }
...
function internalComposerInstall(modulePath) {
  ...
  cp.execSync("composer install --no-dev --ignore-platform-reqs", {
    cwd: modulePath,
    stdio: "ignore",
  });
}
...
function internalComposerBuildExtension() {
  ...
  internalComposerInstall(
    "./build/tmp/files/application/Espo/Modules/" + moduleName
  );
